...
export async function POST(request) {
  ...
  return new Promise((resolve) => {
    db.query('SELECT * FROM users WHERE email = ?', [email], async (err, results) => {
      if (err) {
        resolve(new Response(JSON.stringify({ message: 'DB error' }), { status: 500 }));
        return;
      }

      if (results.length > 0) {
        resolve(new Response(JSON.stringify({ message: 'User already exists' }), { status: 409 }));
        return;
      }
      ...
      db.query(
        ...
        (err) => {
          if (err) {
            resolve(new Response(JSON.stringify({ message: 'DB insert error' }), { status: 500 }));
          } else {
